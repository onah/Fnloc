...
use fnloc::function_analyzer::{analyze_all_files, FunctionAnalysisResult};
...
fn parse_expected_results(file_path: &str) -> Result<Vec<ExpectedResult>, Box<dyn std::error::Error>> {
...
fn test_sample_files_analysis() {
    ...
    assert!(Path::new(test_dir).exists(), "Test sample directory does not exist");
    assert!(Path::new(&format!("{}/sample.rs", test_dir)).exists(), "sample.rs does not exist");
    assert!(Path::new(&format!("{}/module.rs", test_dir)).exists(), "module.rs does not exist");
    ...
    for expected in &expected_results {
        let actual = result_map.get(&expected.name)
            .unwrap_or_else(|| panic!("Function '{}' not found in analysis results", expected.name));
    ...
    println!("✅ All {} function analyses match expected results!", expected_results.len());
}
...
fn test_error_handling() {
    ...
    assert!(result.is_err(), "Expected error for directory with no Rust files");
...
fn test_function_filtering_and_sorting() {
    ...
    for result in &filtered {
        assert!(result.total >= min_lines,
            "Function '{}' has {} lines, which is less than minimum {}", 
            result.name, result.total, min_lines);
    }
